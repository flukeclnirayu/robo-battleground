...
{
	public class StrikerWeapon : MonoBehaviourPunCallbacks
	{
		...
		void Shoot ()
		{
			...
			if (Physics.Raycast(t_spawn.position, t_spawn.forward, out t_hit, 1000f, canBeShot))
			{
				...
				if (photonView.IsMine)
				{
					...
					{
						t_hit.collider.gameObject.GetPhotonView().RPC("TakeDamage", RpcTarget.All, 15);
					}
				}
			}
		}
		...
		{
			Debug.Log(p_damage);
			GetComponent<Striker>().TakeDamage(p_damage);
		}
	}
}
